...
- [ ] Confirm `priority` column on PillCountTxnEntity (Neel said it exists; not visible in entity as of 2026-05-26 — may be in a migration not yet merged, or on a different table). If missing, Disp. High Priority card stays stubbed at 0.
- [ ] List of `drugType` string values that count as "Controlled" (CII/CIII/CIV/CV?). Stubbed at 0 until provided.
